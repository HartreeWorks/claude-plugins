...
2. **LLM engine**: Spawns the `proofreader:proofreader` agent which auto-applies safe corrections and flags style/clarity as suggestions
...
```bash
cd "${CLAUDE_PLUGIN_ROOT}" && npx tsx scripts/proofread.ts "<file_path>" --engine spellcheck
```
